...
def step13(input_dir: str = 'data', output_folder_path: str = 'data'):

    print('Step 13', end='\t')

    bootstr_averages_path = os.path.join(input_dir, 'Bootstrapped_arm_averages.tsv')
    averages_path = os.path.join(input_dir, 'Arm_averages.tsv')
    output_file_path = os.path.join(output_folder_path, 'Arm_Pvalues_cohorts.tsv')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='This script takes ' +
                                                 'Bootstrapped_arm_averages.tsv and Arm_averages.tsv,'
                                                 + '\n' + 'and calculates p-values among cohorts for each arm' + '\n' +
                                                 'and save the result in Arm_Pvalues_cohorts.tsv file' +
    ...
    step13(args.input_dir, args.output_folder)
