...
def step15(input_dir: str = 'data', output_folder_path: str = 'data'):

    print('Step 15', end='\t')

    p_vals_path = os.path.join(input_dir, 'Arm_Pvalues_cohorts.tsv')
    output_file_path = os.path.join(output_folder_path, 'Arm_drivers_FDR5_cohorts.tsv')
    ...
    if not os.path.isfile(output_file_path):
        ...
        p_vals_df = pd.read_csv(p_vals_path, sep='\t', dtype=str)
        ...
        for key in bool_dict:
            ...
            if bool_dict[key]:
                if pos_neg_dict[key] == '+':
                    p_vals_df_copy.loc[cohort_id, arm] = 'DAG'
                elif pos_neg_dict[key] == '-':
                    p_vals_df_copy.loc[cohort_id, arm] = 'DAL'
            else:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='This script takes ' +
                                                 'Arm_Pvalues_cohorts.tsv, does BH correction on that'+ '\n' +
                                                 'and save the result in Arm_drivers_FDR5_cohorts.tsv file' +
    ...
    step15(args.input_dir, args.output_folder)
